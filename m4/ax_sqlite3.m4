...
#  error SQLite version is too old
        ...
        if test "$success" = "yes"; then
            ...
            ac_sqlite3_header_path="$ac_sqlite3_path/include/$ac_sqlite3_header"
 			...
 			AC_MSG_CHECKING([if SQLite3 version is >= $sqlite3_version_req])
            ...
            if test "x$ac_sqlite3_header_path" != "x"; then
                ...
                if test $ac_sqlite3_version != ""; then
                    ...
                    sqlite3_version_shorten=`expr $ac_sqlite3_version : '\([[0-9]]*\.[[0-9]]*\)'`
                    sqlite3_version_major=`expr $ac_sqlite3_version : '\([[0-9]]*\)'`
                    sqlite3_version_minor=`expr $ac_sqlite3_version : '[[0-9]]*\.\([[0-9]]*\)'`
                    sqlite3_version_micro=`expr $ac_sqlite3_version : '[[0-9]]*\.[[0-9]]*\.\([[0-9]]*\)'`
                    if test "x$sqlite3_version_micro" = "x" ; then
                      sqlite3_version_micro="0"
                    fi
                    sqlite3_version_number=`expr $sqlite3_version_major \* 1000000 \
                                   \+ $sqlite3_version_minor \* 1000 \
                                   \+ $sqlite3_version_micro`
                    sqlite3_version_check=`expr $sqlite3_version_number \>\= $sqlite3_version_req_number`
                else
                    AC_MSG_WARN([Can not find SQLITE_VERSION macro in sqlite3.h header to retrieve SQLite version!])
                fi
            fi
            if test "$sqlite3_version_check" = "1"; then
                AC_MSG_RESULT([yes])
            else
                AC_MSG_RESULT([no])
            fi
