...
#  error Oracle oci.h header not found
        ...
        if test "$oci_header_found" = "yes"; then
			...
			dnl Add -lnnz11 (respectively -lnnz10) flag to Oracle >= 11.x (respectively >= 10.x)
			oracle_version_major=`cat $oracle_include_dir/oci.h \
                             | grep '#define.*OCI_MAJOR_VERSION.*' \
                             | sed -e 's/#define OCI_MAJOR_VERSION  *//' \
                             | sed -e 's/  *\/\*.*\*\///'`
			if test "$oracle_version_major" = "11"; then
				oci_libs="-lnnz11 $oci_libs"
        		LIBS="$oci_libs $LIBS"
			elif test "$oracle_version_major" = "10"; then
				oci_libs="-lnnz10 $oci_libs"
        		LIBS="$oci_libs $LIBS"
			fi
