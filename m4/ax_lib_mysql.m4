...
[
    ...
    if test "$want_mysql" = "yes"; then
        ...
        if test -z "$MYSQL_CONFIG" ; then
            ...
            if test -z "$MYSQL_CONFIG" ; then
            	AC_MSG_ERROR([Could find neither mysql_config nor mysql_config5])
            fi
        dnl MYSQL_CONFIG is provided by user but it does not point to executable file
        elif test ! -x "$MYSQL_CONFIG"; then
        ...
        if test "$MYSQL_CONFIG" != "no"; then
            MYSQL_CFLAGS=$($MYSQL_CONFIG --cflags | sed 's/-@<:@^IUD@:>@@<:@^ @:>@*//g')
