 ...
 *
 *
 * $Id: cutil_inline_runtime.h 1784 2012-02-21 10:34:58Z pkestene $
 */
/*
...
#define cutilSafeCallMpi(err,rank)   __cudaSafeCallMpi   (err, rank, __FILE__, __LINE__)
...
inline void __cudaSafeCallMpi( cudaError err, int rank, const char *file, const int line )
{
    if( cudaSuccess != err) {
		FPRINTF((stderr, "[MPI rank %4d] %s(%i) : cudaSafeCall() Runtime API error : %s.\n",
			 rank, file, line, cudaGetErrorString( err) ));
        exit(-1);
    }
}
