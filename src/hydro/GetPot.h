/**
 * \file GetPot.h
 * \brief A very simple header-only configuration file and
 * command-line parser utility.
 *
 * \note http://getpot.sourceforge.net/
 */
...
GetPot::__record_argument_request(const std::string& Name)
{
    ...
    victorate(std::string, STree, it)
      if( std::find(_requested_sections.begin(), _requested_sections.end(), *it) == _requested_sections.end() )
...
{ (void) Requested; __set_variable(VarName, __double2string(Value).c_str()); }

inline void 
GetPot::set(const char* VarName, const int Value, const bool Requested /* = yes */)
{ (void) Requested; __set_variable(VarName, __int2string(Value).c_str()); }
