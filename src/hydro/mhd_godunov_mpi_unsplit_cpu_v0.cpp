...
namespace hydroSimu {
  ...
  // =======================================================
  // =======================================================
  void MHDRunGodunovMpi::godunov_unsplit_cpu_v0(HostArray<real_t>& h_UOld, 
						...
						real_t dt, int nStep)
  {
	...
	for (int i=ghostWidth; i<isize-ghostWidth+1; i++) {
	  ...
	  // in current cell
	  iG   = i + nx*myMpiPos[0];
	  xPos = ::gParams.xMin + dx/2 + (iG-ghostWidth)*dx;
	  ...
	  // in current cell
	  iG   = i + nx*myMpiPos[0];
	  xPos = ::gParams.xMin + dx/2 + (iG-1-ghostWidth)*dx;
	...
	for (int j=ghostWidth; j<jsize-ghostWidth+1; j++) {
	  for (int i=ghostWidth; i<isize-ghostWidth+1; i++) {
	    ...
	    for (int kc=k; kc>=k-1; kc--) {
	      for (int jc=j; jc>=j-1; jc--) {
		for (int ic=i; ic>=i-1; ic--) {
  ...
  } // MHDRunGodunovMpi::godunov_unsplit_cpu_v0
